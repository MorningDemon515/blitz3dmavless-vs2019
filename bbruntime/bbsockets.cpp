...
int UDPStream::recv(){
	...
	for(;;){
		...
		n = ::recvfrom(sock, (char*)in_buf.data(), sz, 0, (sockaddr*)&in_addr, &len);//n=::recvfrom( sock,in_buf.begin(),sz,0,(sockaddr*)&in_addr,&len );
...
int UDPStream::send( int ip,int port ){
	...
	int n = ::sendto(sock, (char*)out_buf.data(), sz, 0, (sockaddr*)&out_addr, sizeof(out_addr));//int n=::sendto( sock,out_buf.begin(),sz,0,(sockaddr*)&out_addr,sizeof(out_addr) );
	if( n!=sz ) return e=-1;
	out_buf.clear();
	return sz;
}
